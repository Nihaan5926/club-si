...
if (DATABASE_URL) {
  ...
  (async () => {
    try {
      ...
      `);
      console.log('Postgres DB ready for Futsal App (with Auto-Advancing Groups)');
    } catch (e) {
...
// Calculate Standings Helper
function calculateStandings(teams, matches) {
  let standings = teams.map(t => ({ id: t.id, name: t.name, played: 0, won: 0, drawn: 0, lost: 0, gf: 0, ga: 0, gd: 0, pts: 0 }));
  
  const completedGroupMatches = matches.filter(m => m.status === 'completed' && m.stage === 'group');
  
  completedGroupMatches.forEach(m => {
    let t1 = standings.find(s => s.id === m.team1_id);
    let t2 = standings.find(s => s.id === m.team2_id);
    if(!t1 || !t2) return;
...
// GET: Dashboard Data (Teams, Players, Matches, Standings)
...
// DELETE / UPDATE STATS endpoints remain largely the same, trimmed for brevity
...
// POST: Update Match State & Auto-Advance Logic
app.post('/api/matches/:id/action', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (action === 'complete') {
      ...
      // If there are group matches, and ALL of them are completed, trigger Semi-Finals
      if (allMatches.length > 0 && liveGroupMatches.length === 0) {
        // Check if semis already exist to prevent duplicate creation
        let existingSemis;
        ...
        if (existingSemis.length === 0 && allTeams.length >= 4) {
          ...
          // Matchups: 1st vs 4th, 2nd vs 3rd
          if (pool) {
            await pool.query('INSERT INTO matches (team1_id, team2_id, status, stage) VALUES ($1, $2, $3, $4), ($5, $6, $7, $8);', 
              [top4[0].id, top4[3].id, 'live', 'semi', top4[1].id, top4[2].id, 'live', 'semi']);
          } else {
            mem.matches.push({ id: mem.matchIdSeq++, team1_id: top4[0].id, team2_id: top4[3].id, team1_score: 0, team2_score: 0, status: 'live', stage: 'semi', created_at: new Date() });
            mem.matches.push({ id: mem.matchIdSeq++, team1_id: top4[1].id, team2_id: top4[2].id, team1_score: 0, team2_score: 0, status: 'live', stage: 'semi', created_at: new Date() });
          }
          console.log("Auto-generated Semi-Finals based on standings.");
        }
      }
    }
    res.json({ ok: true });
